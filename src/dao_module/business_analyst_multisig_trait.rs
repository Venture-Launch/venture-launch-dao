...
use squads_multisig::{
    client::{self, config_transaction_create, config_transaction_execute, multisig_create_v2, proposal_create, vault_transaction_create, vault_transaction_execute, ConfigTransactionCreateAccounts, ConfigTransactionCreateArgs, ConfigTransactionExecuteAccounts, MultisigCreateAccountsV2, MultisigCreateArgsV2, ProposalCreateArgs, VaultTransactionCreateAccounts, VaultTransactionExecuteAccounts}, pda::{get_proposal_pda, get_transaction_pda}, squads_multisig_program, state::{
        ConfigAction, Member, Permission, Permissions, TransactionMessage
...
#[async_trait]
pub trait BusinessAnalystMultisigTrait<Args = BaseMultisigCreateArgs> : BaseMultisigTrait<Args, Error = BaseMultisigError>{

    fn instruction_create_multisig(&self, members: &[Member], threshold: u16, time_lock: u32) -> Instruction;
    async fn transaction_create_multisig(&self, members: &[Member], threshold: u16, time_lock: u32) -> Result<Transaction, Self::Error>;

    async fn instructions_add_member(&self, adder: Pubkey, new_member: Member) -> Result<Instruction, Self::Error>;
    async fn instructions_remove_member(&self, remover: Pubkey, old_member_pubkey: Pubkey) -> Result<Instruction, Self::Error>;
    async fn instruction_transfer_from_vault(&self, sender: Pubkey, receiver: Pubkey, lamports: u64) -> Result<Instruction, Self::Error>;
    async fn instruction_proposal_create(&self, creator: Pubkey)  -> Result<Instruction, Self::Error>;
    async fn instruction_config_transaction_execute(&self, executer: Pubkey) -> Result<Instruction, Self::Error>;
    async fn instruction_vault_transaction_execute(&self, sender: Pubkey, receiver: Pubkey, lamports: u64) -> Result<Instruction, Self::Error>;
    async fn instruction_change_threshold(&self, changer: Pubkey, new_threshold: u16) -> Result<Instruction, Self::Error>;

    async fn transaction_add_member(&self, adder: Pubkey, new_member: Member) -> Result<Transaction, Self::Error> {
        let ix = self.instructions_add_member(adder, new_member).await?;

        Ok(self.get_transaction_from_instructions(adder, &[ix]).await?)
    }
    async fn transaction_remove_member(&self, remover: Pubkey, old_member_pubkey: Pubkey) -> Result<Transaction, Self::Error> {
    ...
    async fn transaction_transfer_from_vault(&self, sender: Pubkey, receiver: Pubkey, lamports: u64) -> Result<Transaction, Self::Error> {
    ...
    async fn transaction_proposal_create(&self, creator: Pubkey)  -> Result<Transaction, Self::Error> {
        let ix = self.instruction_proposal_create(creator).await?;

        Ok(self.get_transaction_from_instructions(creator, &[ix]).await?)
    }
    async fn transaction_config_transaction_execute(&self, executer: Pubkey) -> Result<Transaction, Self::Error> {
    ...
    async fn transaction_vault_transaction_execute(&self, sender: Pubkey, receiver: Pubkey, lamports: u64) -> Result<Transaction, Self::Error> {
        let ix = self.instruction_vault_transaction_execute(sender, receiver, lamports).await?;

        Ok(self.get_transaction_from_instructions(sender, &[ix]).await?)
    }
    async fn transaction_change_threshold(&self, changer: Pubkey, new_threshold: u16) -> Result<Transaction, Self::Error> {
...
mod tests {
    ...
    use solana_sdk::{native_token::LAMPORTS_PER_SOL, signature::{Keypair, Signature}};
    use tokio;
    ...
    async fn create_multisig_no_members() -> Result<(), Box<dyn Error>> {
        ...
        let base_multisig = get_base_multisig(&rpc_client, &create_key, &creator, &[]).await.unwrap();
    ...
    async fn add_member() -> Result<(), Box<dyn Error>> {
        ...
        let mut tx = ba_multisig.transaction_add_member(creator.pubkey(), new_member).await.unwrap();
        transaction_sign_and_send(&mut tx, &[&creator], &rpc_client).await.unwrap();

        let mut tx = ba_multisig.transaction_proposal_create(creator.pubkey()).await.unwrap();
        transaction_sign_and_send(&mut tx, &[&creator], &rpc_client).await.unwrap();

        let mut tx = ba_multisig.transaction_proposal_approve(creator.pubkey()).await.unwrap();
        transaction_sign_and_send(&mut tx, &[&creator], &rpc_client).await.unwrap();

        let mut tx = ba_multisig.transaction_config_transaction_execute(creator.pubkey()).await.unwrap();
        transaction_sign_and_send(&mut tx, &[&creator], &rpc_client).await.unwrap();
    ...
    async fn change_threshold() -> Result<(), Box<dyn Error>> {
        ...
        let _ = airdrop(&rpc_client, &creator.pubkey(), 1).await?;
        let base_multisig = get_base_multisig(&rpc_client, &create_key, &creator, &[new_member]).await.unwrap();
    ...
    async fn vault_transaction_member_approve() -> Result<(), Box<dyn Error>> {
        ...
        assert_eq!(rpc_client.get_balance(&multisig.get_vault_pda()).await.unwrap(), 1 * LAMPORTS_PER_SOL);
        Ok(())
    }
}